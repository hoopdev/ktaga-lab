...
class PNABase(VisaInstrument):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.add_parameter(
            ...
            get_cmd="CALC:FILT:TIME:STAT?",
            set_cmd="CALC:FILT:TIME:STAT {}",
            val_mapping={True: "1", False: "0"},
        )
        self.add_parameter(
            "gating_start",
            label="Gating Start time",
            get_cmd="CALC:FILT:TIME:STAR?",
            get_parser=float,
            set_cmd="CALC:FILT:TIME:STAR {:.4e}",
            unit="s",
            vals=Numbers(min_value=0, max_value=10e-6),
        )
        self.add_parameter(
            "gating_stop",
            label="Gating Stop time",
            get_cmd="CALC:FILT:TIME:STOP?",
            get_parser=float,
            set_cmd="CALC:FILT:TIME:STOP {:.4e}",
            unit="s",
            vals=Numbers(min_value=0, max_value=10e-6),
        )
    ...
    def traces(self) -> ChannelList:
        ...
        for trace_name in parlist[::2]:
            ...
            pna_trace = PNATrace(self, f"tr{trace_num}", trace_name, trace_num)
    ...
    def gating_on(self) -> None:
        """
        Turn on trace gating
        """
        self.gating_enabled(True)

    def gating_off(self) -> None:
        """
        Turn off trace gating
        """
